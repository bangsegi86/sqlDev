...
powershell -NoProfile -Command ^
  ...
  2>nul
if not errorlevel 1 (
  echo 관리자가 이미 백그라운드에서 실행 중입니다. 브라우저를 엽니다...
...
REM ── 관리자를 백그라운드로 시작 (CMD 창 없음) ──
echo SQLDev 관리자를 백그라운드로 시작합니다...
wscript //nologo "%~dp0start-manager.vbs"

REM ── 관리자 HTTP 서버가 준비될 때까지 대기 (최대 10초) ──
set READY=0
...
if !READY!==0 (
  echo.
  echo [경고] 관리자 시작을 확인하지 못했습니다. 수동으로 http://localhost:3100/ 을 열어보세요.
)
