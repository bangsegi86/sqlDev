# SQLDev — Oracle / PostgreSQL DB 관리 도구

DBeaver와 유사한 데이터베이스 웹 관리 도구입니다.  
브라우저에서 **Oracle** 또는 **PostgreSQL** DB에 접속하여 테이블 조회, 프로시저/함수 분석, SQL 실행 등을 할 수 있습니다.

> 연결 추가/편집 화면에서 **데이터베이스 종류**(Oracle / PostgreSQL)를 선택할 수 있습니다.
> - Oracle: **서비스명**, 기본 포트 `1521`
> - PostgreSQL: **데이터베이스명**, 기본 포트 `5432` (별도 드라이버 설치 불필요 — `pg` 모듈 내장)
...
1. 좌측 상단 **`+ 새 연결`** 버튼 클릭
2. **데이터베이스 종류** 선택 (Oracle / PostgreSQL)
3. 접속 정보 입력
   ...
   **PostgreSQL**
   ...
   | 항목 | 예시 | 설명 |
   |------|------|------|
   | Host | `192.168.1.100` | PostgreSQL 서버 IP 또는 도메인 |
   | Port | `5432` | 기본값 5432 |
   | 데이터베이스명 | `postgres` | 접속할 데이터베이스 |
   | Username | `postgres` | DB 계정 |
   | Password | `••••` | DB 비밀번호 |

4. **`연결 테스트`** 클릭 → 성공 메시지 확인
5. **`저장`** 또는 **`저장 & 연결`** 클릭
