...
// Split a SQL script by ';' that are not inside string literals
function splitStatements(script) {
  ...
  for (let i = 0; i < script.length; i++) {
    const ch = script[i];
    if (!inStr && ch === "'") { inStr = true; buf += ch; }
    else if (inStr && ch === "'" && script[i + 1] === "'") { buf += "''"; i++; }
    else if (inStr && ch === "'") { inStr = false; buf += ch; }
    else if (!inStr && ch === ';') {
      const s = buf.trim();
